...
#define AVRLPT_VID 0x1209
#define AVRLPT_PID 0x4153
...
static bool Write(AVRLPT dev, AVRLPT_COMMAND request, uint16_t value, uint16_t index, uint8_t *data, uint16_t size)
{
    LONG transferred;
...
static bool Read(AVRLPT dev, AVRLPT_COMMAND request, uint16_t value, uint16_t index, uint8_t* data, uint16_t *size)
{
    LONG transferred;
    WINUSB_SETUP_PACKET packet =
    {
...
AVRLPT AvrLpt_Open()
{
    ...
    if (FindOpenDevice(AVRLPT_VID, AVRLPT_PID, L"a.sakharov", L"Virtual EPP", &devHandle, &devWinUsbHandle))
    {
        result = malloc(sizeof(struct AVRLPT_t));
        if(!result)
        {
...
{
    uint16_t size = sizeof(AVRLPT_version);
